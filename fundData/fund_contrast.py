...
for code in fund_codes:
    # try:
        ...
        for key in ['今年来', '近1周', '近1月', '近3月', '近6月', '近1年', '近2年', '近3年']:
            output_data[key].append(fund_info[key])
        
        # 费率信息
        fee_res = requests.get(f'https://fundf10.eastmoney.com/jbgk_{code}.html')
        fee_soup = BeautifulSoup(fee_res.text, 'html.parser')
        output_data['管理费率'].append(
            fee_soup.find('th', string='管理费率').find_next('td').text.split('%')[0] + '%')
        output_data['托管费率'].append(
            fee_soup.find('th', string='托管费率').find_next('td').text.split('%')[0] + '%')
        
        # 其他信息
        output_data['年化跟踪误差'].append(
            re.search(r'年化跟踪误差：(\d+\.\d+%)', 
                     soup.find('a', string='年化跟踪误差').parent.text).group(1))
        output_data['基金规模'].append(
            re.search(r'(\d+\.\d+)亿元', 
                     soup.find('a', string='规模').parent.text).group(0))
